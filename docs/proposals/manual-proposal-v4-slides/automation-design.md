...
> 注（2026-06）: 本番運用は **Slack スレッド中心の手作業フロー** に切り替えた。現行の実運用手順は
> `../manual-slide-operations.md` を参照。本 MD が前提とするスプシ常駐監視（watcher / sheets_client /
> drive_client、19 列スキーマ）は当面 critical path 外（保留中の将来案）。状態管理を自動化したく
> なったときに再評価する。
