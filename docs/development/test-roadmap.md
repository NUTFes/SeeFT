...
フェーズ1 以降の各フェーズは、いきなりテストコードを書くのではなく、次のサイクルで回す（フェーズ0 だけは基盤作業なので設計ステージはない）。

```text
設計起案（AI・強いモデル）→ 設計レビュー（人間: MT または PR）→ 実装量産（AI・Sonnet で可）→ 検証（CI + CodeRabbit）
```

- **設計起案**: 対象関数ごとに「テストケース表」を AI が起案する。表の形式は、ケース名 / 分類（正常系・境界値・異常系）/ 入力 / 期待値 / 根拠 / 要判断フラグ。期待値は起案のまま使わず、使い捨てテストを実際に実行して実挙動と突き合わせてから設計書に載せる（推測の期待値を設計書に残さない）。
- **設計レビュー**: 人間がレビューするのはこの表であって、テストコードではない。見る点は2つ。ケースの過不足（この入力パターンが漏れていないか）と、「要判断」ケースの裁定（現状挙動をバグとして直すか、仕様として固定するか）。後者はこのロードマップの方針「現状の動作を正解にする」の唯一の例外判断であり、MT の議題にそのまま使える。
- **実装量産**: 承認済みのケース表をプロンプトに貼って実装させる。設計が確定していれば機械的な作業なので、Sonnet クラスのモデルで十分。
- **検証**: CI の go test と CodeRabbit。人間は diff がケース表と一致しているかを見る。

### Claude への依頼方法

- 依頼単位は 1 issue = 1〜2 関数。生成は安いがレビューは人間がやるので、レビュー可能な粒度を守る。
- 設計起案のプロンプトに含めるもの: 対象関数のファイルパスと行番号、このロードマップの該当フェーズの節、上記のケース表形式、「現状の動作を正解にする。ただし直感に反する挙動には要判断フラグを付ける」という方針。
- 実装のプロンプトに含めるもの: 承認済みのケース表、テストの置き場所（対象と同一パッケージ内の `*_test.go`）、テーブル駆動テストで書くこと。

フェーズ1 の設計書第1弾は `docs/development/test-design/phase1-pure-functions.md` にある（この節のサイクルを実際に回して作成したもの）。
