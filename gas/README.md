...
| `rescue/` | `45th_レスキュー` のプロジェクト（`doPost` のウェブアプリでスプシに追記し、押し直しによる重複はまとめる。`onChange` でDB更新） | 2026-09-19 | 45th の現行。この日時点でライブと全3ファイル一致。ウェブアプリはバージョン固定（この日時点でバージョン4）なので、`clasp push` のあと同じデプロイIDへ `clasp deploy -i` しないと本番に反映されない |
